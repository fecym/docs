...
- 父节点：A 为 B 和 C 的父节点
- 兄弟节点：同一个父节点的子女节点互称为兄弟；B 和 C 互为兄弟节点
...
接下来汇总一下工作中常用的一些树的基本操作

测试的树的数据结构就拿 `elementUI` 中级联选择器里面的数据来测试。
...
5. 深度优先：访问完一颗子树再去访问后面的子树，而访问子树的时候，先访问根再访问根的子树，称为先序遍历；先访问子树再访问根，称为后序遍历。
...
思路是一致的，但是这个要额外记录一下访问过的节点，最终返回访问过的所有节点
...
树的遍历是很基础的操作，树几乎所有的操作都是基于遍历来实现的，上面都是用深度优先来操作的，给树增加 level 用广度优先遍历更为合适
...
处理完之后的[数据地址](https://chengyuming.cn/file/treeLevel.json)

## 给树增加 parentId

给树增加父级 id 又得用到深度优先了
给一颗没有 parentId 的树增加 parentId，方便下面的树转数组的使用
...
处理完之后的[数据地址](https://chengyuming.cn/file/treeParentId.json)
...
工作中可能有这种数组转成树的需求，其实实现也不复杂，只需要遍历两次树，第一次把所有的 id 存到一个 map 里，第二次遍历处理这个数组
...
二叉树的结构是 `{ val: 0, left: null, right: null }` 这样的一个结构，left 和 right 指针指向下一层，val 对应当前的值，二叉树虽然在前端工作中基本上用不到，但是理解二叉树，对于学习算法是一个很重要的开始，后面会讲一下二叉树的概念，然后介绍几个面试中经常被问到的关于二叉树的问题
...
要学习二叉树相关的知识就要先生成一个这样的结构，因为二叉搜索树是可以与有序数组互转的

实现思路也很简单：我们拿有序数组中间的值作为每次递归的 root 节点，为他的 left 节点创建取数组中 0 到 middle 的地方，right 节点创建取 middle + 1 到数组长度的地方皆可
...
function array2BST(array) {
  ...
  root.left = array2BST(arr.slice(0, middle));
  root.right = array2BST(arr.slice(middle + 1, len));
  return root;
}
```
...
3. 还有一种方法就是记录最大值和最小值，然后与当前的值进行比较，只要在 min 和 max 之间那就是合法的，否则就不是一个二叉搜索树
...
二叉树按照之字进行遍历，跟上面的二叉树遍历类似，上面的遍历是每一层都是按照从左到右的顺序进行遍历

之字遍历是第一层按从左到右的顺序遍历，第二层反过来按照从右到左的顺序遍历，第三次正着，第四层反着...，以此类推

我们发现其实就是偶数层从左到右，奇数层从右到左(从 0 开始数数哦)，所以只需要维护一个自增变量即可
...
最小深度需要考虑没有左子树或者右子树的情况，所以不能直接用 Math.min 取值
...
然后递归的访问左子树和右子树，如果左子树返回为空那么就不用在继续查下去了直接查右子树就可以了，如果右子树返回为空也是如此，递归的重复这个步骤，当发现左子树和右子树都可以查得到的时候就找到公共祖先了，如果都没找到那么根节点就是公共祖先
...
如果是一棵二叉搜索树的话更简单，只需要判断当前的值和 p、q 的大小即可
